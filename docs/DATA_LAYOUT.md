...
| Prowlarr, FlareSolverr, Jellyseerr, Wizarr, Caddy | — | — | No acceden a `data/`. |
...
`bash scripts/bootstrap.sh` ya configura todo esto por vos vía API. Si en algún momento necesitás tocarlo a mano, los paths correctos son:

- **Sonarr**: Settings → Media Management → Root Folders → add `/data/media/series`
...
- **Bazarr**: Settings → Sonarr/Radarr → Connect (usa la misma API key, no hace falta folder mapping porque los paths ya coinciden entre containers)

> Nota: los root folders van bajo `/data/media/...`, no directo en `/data/...` — Sonarr y Radarr ven todo `/data` montado (para los hardlinks), pero la biblioteca final vive en el subdirectorio `media/`.
...
# Reiniciar y re-aplicar toda la configuración (idempotente)
docker compose up -d
bash scripts/bootstrap.sh
```

`bootstrap.sh` deja los Root Folders de Sonarr/Radarr en `/data/media/series` y `/data/media/movies` respectivamente. Jellyfin y Bazarr no necesitan cambios (su target de mount `/data/media` no cambió).
